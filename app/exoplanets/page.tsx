      ...
      {isLoading ? (
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mt-3">
          {Array.from({ length: limit || 8 }).map((_, i) => (
            <div key={i} className={`flex flex-col justify-between h-[220px] bg-bg-card/20 border border-border/30 rounded-xl p-4 animate-pulse ${limit && i > 1 ? 'hidden sm:flex' : ''} ${limit && i > 2 ? 'hidden lg:flex' : ''} ${limit && i > 3 ? 'hidden xl:flex' : ''}`}>
