...
export function DitheringShader({
  ...
  useEffect(() => {
    ...
    // Set canvas html size via ResizeObserver or window size dynamically
    const updateSize = () => {
        if (!canvas) return;
        const rect = canvas.parentElement?.getBoundingClientRect();
        canvas.width = rect ? rect.width : (typeof width === "number" ? width : 800);
        canvas.height = rect ? rect.height : (typeof height === "number" ? height : 800);
