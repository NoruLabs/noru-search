...
const start = c.indexOf('<div className={layout === "compact"');
const end = c.lastIndexOf('      </div>\n\n      {!showViewAll');

const newContent = `<div className={layout === "compact" ? "flex flex-col gap-4 mt-3 h-full" : layout === "carousel" ? "relative w-full mt-3 block" : "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-3"}>
        {layout === "carousel" ? (
          <div className="relative w-full px-0 sm:px-6">  
            <Carousel>
              <CarouselContent className="pb-4 pt-2 -ml-4 flex">
                {displayItems.map((item) => (
                  <CarouselItem key={\`carousel-\${item._type}-\${item.id}\`} className="basis-full sm:basis-1/2 lg:basis-1/3 min-w-0 pl-4">
                    <DataCard
...
fs.writeFileSync('app/news/SpaceNewsPanel.tsx', c.substring(0, start) + newContent + c.substring(end) + "\n");
